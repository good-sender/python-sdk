...
toml="$SDK_DIR/pyproject.toml"
if [[ -f "$toml" ]]; then
  # The python generator writes `license = "NoLicense"` regardless of licenseInfo
  # config. Force MIT.
  sed -i.bak -E 's/^license = "NoLicense"$/license = "MIT"/' "$toml"
  sed -i.bak -E 's/^description = ".*"$/description = "Official GoodSender API client for Python"/' "$toml"
  sed -i.bak -E 's/^keywords = \[.*\]$/keywords = ["goodsender", "email", "api", "sdk", "consent", "transactional"]/' "$toml"
  rm -f "$toml.bak"
fi
