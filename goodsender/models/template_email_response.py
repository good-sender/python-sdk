...
class TemplateEmailResponse(BaseModel):
    ...
    """ # noqa: E501
    status: StrictStr = Field(description="\"sent\" if the email was sent, \"declined\" if the recipient has opted out")
    __properties: ClassVar[List[str]] = ["status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['sent', 'declined']):
            raise ValueError("must be one of enum values ('sent', 'declined')")
        return value
