...
class QuotaExceededError(BaseModel):
    ...
    kind: StrictStr = Field(description="Whether the daily or monthly quota was exhausted.")
    message: StrictStr = Field(description="Human-readable error message.")
    limit: StrictInt = Field(description="Quota limit that was reached.")
    used: StrictInt = Field(description="Number of emails already used against the quota.")
    reset_at: datetime = Field(description="Timestamp at which the quota window resets.", alias="resetAt")
    __properties: ClassVar[List[str]] = ["code", "kind", "message", "limit", "used", "resetAt"]

    @field_validator('code')
    def code_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['quota_exceeded']):
            raise ValueError("must be one of enum values ('quota_exceeded')")
        return value
