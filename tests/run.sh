#!/usr/bin/env bash
# Single-language conformance driver for good-sender/python-sdk.
#
# Usage:
...
build_sdk() { :; }   # python doesn't need a build step; pip install -e is the install
...
install_runner() {
  ...
  if [[ ! -d "$venv" ]]; then
    ...
    "$venv/bin/pip" install --quiet -e "$SDK_DIR" >/dev/null
  fi
}
...
case "$MODE" in
  mock)
    ...
    build_sdk
    install_runner
    "$SCRIPT_DIR/.venv/bin/python" "$SCRIPT_DIR/runner.py"
    ;;
  dev)
    ...
    build_sdk
    install_runner
    "$SCRIPT_DIR/.venv/bin/python" "$SCRIPT_DIR/conformance.py"
    ;;
